...
public class ProcessPage {
    ...
    {
        return findPageNumber(globalVerseIndex);
    }


    public static int getIndexofSurah(String surahName)
    {
        if  (surahName.equals("آل عمران")) return 0;
        return Surahs.valueOf(surahName).ordinal();
    }



    public static int getVerseIndexInSurah(int globalVerseIndex)
    {
        int sum = 0;

        for (int i = 0; i < 114; i++)
        {
            sum += numOfVerseInEverySurah[i];

            if (globalVerseIndex <= sum) {
                break;
            }

        }

        return ( sum - globalVerseIndex) ;
    }

    public static int getGlobalIndexOfVerse(int verseIndexInSurah, int surahIndex )
    {
        ...
        return  versesBefore + verseIndexInSurah;
    }

    private static int findPageNumber(int globalVerseIndex)
    {
        if  (globalVerseIndex >= 6222) return 603;

        int  currentPage = 0 , pageAfter = 1 ;

        for (int index = 0; currentPage < 603 && pageAfter < 604 ; index++, currentPage++ ,pageAfter++)
        {
            if  ((globalVerseIndex >= pageHeaderIndex[currentPage]) && (globalVerseIndex < pageHeaderIndex[pageAfter]))
               break;
        }
